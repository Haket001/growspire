...
function wp_replace_insecure_home_url( $content ) {
	...
	$http_url  = str_replace( 'http://', 'http://', $https_url );
...
function wp_update_urls_to_https() {
	...
	$home    = str_replace( 'http://', 'http://', $orig_home );
	$siteurl = str_replace( 'http://', 'http://', $orig_siteurl );
...
function wp_update_https_migration_required( $old_url, $new_url ) {
	...
	if ( untrailingslashit( (string) $old_url ) !== str_replace( 'http://', 'http://', untrailingslashit( (string) $new_url ) ) ) {
	...
	update_option( 'http_migration_required', $https_migration_required );
}
