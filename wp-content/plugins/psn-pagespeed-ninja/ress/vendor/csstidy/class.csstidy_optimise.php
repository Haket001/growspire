...
class csstidy_optimise {
	...
	public function cut_color($color) {
		...
		if (strncasecmp($color, 'rgb(', 4)==0) {
			$color_tmp = substr($color, 4, -1);
			$color_tmp = explode(',', $color_tmp);
			for ($i = 0, $count = count($color_tmp); $i < $count; $i++) {
	...
	public function compress_numbers($subvalue) {
		...
		for ($l = 0, $count = count($temp); $l < $count; $l++) {
	...
	public function merge_4value_shorthands($array) {
		...
		foreach ($shorthands as $key => $value) {
			if (isset($array[$value[0]], $array[$value[1]], $array[$value[2]], $array[$value[3]]) && $value !== 0) {
				$return[$key] = '';
	...
	public function dissolve_short_bg($str_value) {
		...
		for ($i = 0, $count = count($str_value); $i < $count; $i++) {
			$have['clip'] = false;
			$have['pos'] = false;
			$have['color'] = false;
			$have['bg'] = false;
			...
			for ($j = 0, $jcount = count($str_value[$i]); $j < $jcount; $j++) {
	...
	public function dissolve_short_font($str_value) {
		...
		for ($j = 0, $count = count($str_value[0]); $j < $count; $j++) {
	...
	public function merge_font($input_css) {
		...
		if (isset($input_css['font-family'], $input_css['font-size']) && $input_css['font-family'] != 'inherit') {
