...
class csstidy
{
    ...
    public function load_template($content, $from_file = true)
    {
        ...
        for ($i = 0, $count = count($template); $i < $count; $i++) {
            ...
            switch ($this->status) {
                    ...
                    if ($this->is_token($string, $i) || $pn) {
                        ...
                        if (($string[$i] === '}' || $string[$i] === ';' || $pn) && !empty($this->selector)) {
                            ...
                            while (count($this->sub_value_arr)) {
                                ...
                                if (strpos($this->selector, 'font-face') !== false) {
                ...
                case 'instr':
                    ...
                    if ($_str_char === ')' && ($string[$i] === '(' || $string[$i] === '"' || $string[$i] === '\'') && !$this->escaped($string, $i)) {
                        ...
                        $this->str_char[] = $string[$i] === '(' ? ')' : $string[$i];
                        $this->from[] = 'instr';
                        $this->quoted_string[] = ($string[$i] !== '(' && trim($_cur_string) === '(') ? $_quoted_string : !($string[$i] === '(');
                        break;
                    }
    ...
    {
        return $pos > 0 && $string[$pos - 1] === '\\' && !$this->escaped($string, $pos - 1);
    }
    ...
    public function css_new_selector($media, $selector)
    {
        ...
        if (strncmp($selector, '@font-face', 10) != 0) {
            ...
            if (!$this->css || empty($this->css[$media]))
                return $selector;
    ...
    public function css_new_property($media, $selector, $property)
    {
        ...
        if (!$this->css || empty($this->css[$media][$selector]))
            return $property;
