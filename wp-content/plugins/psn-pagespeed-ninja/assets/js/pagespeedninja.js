...
(function () {
    ...
    jQuery(document).ready(function () {
        ...
        if ($email_popup.length && 'localStorage' in window) {
            var lastTime = localStorage.getItem('pagespeedninja_emailform_popup'),
                now = Date.now();
            if (now - lastTime >= 7 * 24 * 60 * 60 * 1000) {
                setTimeout(function () {
                    localStorage.setItem('pagespeedninja_emailform_popup', now);
                    tb_show('', '#TB_inline?width=500&height=230&inlineId=pagespeedninja_emailform_popup');
                }, 15 * 1000);
            }
            $email_popup.find('form').on('submit', function (e) {
                ...
                jQuery.ajax({
                    type: 'POST',
                    url: 'https://pagespeed.ninja/api/subscribe',
                    data: {
